...
public class ConstInCurrentContextTreeScanner extends BooleanTreeScanner<CompilerTaskContext> {
    ...
    public static boolean isConstInContext(Tree node, CompilerTaskContext context) {

        if (node instanceof MemberSelectTree memberSelectTree) {

            return primitify(memberSelectTree.getExpression().accept(new ConstInCurrentContextTreeScanner(), context));
        }

        if (node instanceof IdentifierTree) {

            return primitify(context.getParent(node).accept(new ConstInCurrentContextTreeScanner(), context));
        }

        return primitify(node.accept(new ConstInCurrentContextTreeScanner(), context));
    }

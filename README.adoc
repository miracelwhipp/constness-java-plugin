...
As opposed to the keyword `final` that restricts write access to the fields, parameters or variables it is applied to, `@Const`
restricts write access to every member of the field, parameter or variable it is applied to. When using this compiler
...
[source,java]
----
include::java-plugin/src/test/cases/error/CastConstAway-11-18.java[tag=cast-const-away]
----
